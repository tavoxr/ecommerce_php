...
Editor::inst( $db, 'Productos' )
	...
	->join(
        Mjoin::inst( 'files' )
            ...
            ->fields(
                Field::inst( 'id' )
                    ->upload( Upload::inst( $_SERVER['DOCUMENT_ROOT'].'/ecommerce_php/uploads/__ID__.__EXTN__' )
                        ->db( 'files', 'id', array(
                        ...
                        ->validator( Validate::fileExtensions( array( 'png', 'jpg', 'jpeg', 'gif','webp' ), "Por favor sube una imagen" ) )
                    )
            )
    )
	->debug(true)
